...
| 要素 | 向き |
|---|---|
| セリフ（吹き出し） | **縦書き**で統一（縦と横を混ぜない） |
| 図解・グラフ・価格の比較・表（図1など） | **横書き**（数字比較は横しか成立しない） |
| ナレーション・注釈・保護者あとがき・免責 | 横書き（noteの本文テキストとして貼る） |

- **吹き出し内の数字は漢数字**（Canvaは縦中横に非対応。縦書きで自然に読める形にする）:
  例「十年後」「二十四歳」「十万円」「十円→十二円→十五円」「四十二年ずっと十円」
  ※算用数字（10, 12, 15）を残すのは図解・グラフ・表など横書き要素だけ
