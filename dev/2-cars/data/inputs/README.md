# 手動入力（取得失敗時のフォールバックのみ）

通常は `collect.py` が自販連・全軽自協のサイトからエクセルを自動取得する。
**サイト構造が変わって `⚠️ 台数データが取れませんでした` が出たときだけ**、ここに手貼りする。

自販連・全軽自協の車名別データから、上位を `YYYY-MM.json` に置く。
対象月 `YYYY-MM` は**前月**（例: 8日に9月分は出ないので、8日実行なら 8月分＝`2026-08`）。
`2026-07.json` が記入済みの例（自動取得が動くので実際には未使用だが、形の見本として残す）。
