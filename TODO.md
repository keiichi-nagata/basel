...
- [ ] **毎朝6:15頃kabuステーションが強制ログアウトする件（2026-09-15判明）→完全自動化を目指す方針**:
      社長が「パスキー」を有効化してOTP無しでログインできるか確認中（`kabu-api-notes.md`参照）。
      OTP不要になれば、kabuステーション.exeをタスクスケジューラで毎朝自動起動するだけで済む
      （認証情報はkabuステーション自身が保持、スクリプト側では一切扱わない）
