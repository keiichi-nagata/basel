...
- [x] 三菱UFJ eスマート証券の口座開設が完了（2026-09-12）。信用取引口座は別用途で申込み中
      （bot v1の売買区分は**現物取引**と確認済み。`trade/strategy.md` 判断ログ参照）
- [ ] このPCにkabuステーションをインストール（社長のみ実施。Claude/エージェントは一切関与しない）
- [ ] API有効化・APIキー発行（要件はkabuステーションのヘルプ/公式サイトで確認）
