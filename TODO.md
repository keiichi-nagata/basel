...
- [x] **毎朝6:15頃kabuステーションが強制ログアウトする件 → 自動ログインの雛形を実装（2026-09-16）**:
      パスキー（Windows Hello PIN）でOTPは不要になったが、PIN入力の自動化が必要と判明。
      PyWinAutoで起動〜パスキーログイン〜PIN入力までを行う`scripts/auto_login_kabu.py`を作成
      （`投資/daytrade-bot`にコミット済み。参考: https://note.com/hraps/n/n2bbbcf8ffca0）
- [ ] `auto_login_kabu.py`の動作確認（社長が実施）: PIN・実行ファイルパスを`.env`に設定 →
      pywinautoインストール → 手動実行して「パスキーでログイン」ボタン・PIN入力欄が
      正しく見つかるか確認（見つからない場合はUI要素名をコード内コメントの手順で調整）
- [ ] 動作確認できたら、タスクスケジューラに登録（毎朝6:45頃、**「最上位の特権で実行する」必須**）
