...
- [ ] 保険スクエアbang!（自動車保険、別ASP）は引き続き承認待ち。下りたら発行URLを `affiliates.json` に貼る
...
- [ ] 2026-09-15 2日目の本番デイトレ（7552ハピネット、手動買い100株@3,889円）→ 時間指定W指値で
      決済発注済み、結果待ち。確定したら`daytrade_trades`・`trade/journal/2026-09.md`に損益を記入
- [ ] **修正後、あらためて`run_live_once.py`で自動発注（買い）の本番トライアルを再実施**（次回の
      デイトレ実施日に）。exchange=9で実際にCode:100368が解消するか検証する
- [ ] 決済（売り）の自動化は当面見送り。手動決済の挙動を踏まえて設計を検討
- [x] **毎朝6:15頃kabuステーションが強制ログアウトする件 → 自動ログインを実装（2026-09-16）**:
      パスキー（Windows Hello PIN）方式は、PIN入力画面がWindowsのセキュアデスクトップに表示され
      自動化不可能と判明（UIA検出・SendInput両方で実機テストして確認。意図的なセキュリティ境界
      のため不採用）。**口座番号＋パスワード＋ワンタイムパスワード（Gmail・IMAP自動取得）方式**に
      変更した`scripts/auto_login_kabu.py`を作成（`投資/daytrade-bot`にコミット済み）
- [ ] `auto_login_kabu.py`の動作確認（社長が実施）: `.env`に`KABU_ACCOUNT_NUMBER`・
      `KABU_LOGIN_PASSWORD`・`GMAIL_ADDRESS`・`GMAIL_APP_PASSWORD`を設定
      （Gmailは2段階認証プロセス→アプリパスワード発行が必要）→ 実行して口座番号・パスワード・
      OTP入力が正しく進むか確認（各画面のフォーカス位置・OTPメールの絞り込み条件は調整が必要な可能性）
- [ ] 動作確認できたら、タスクスケジューラに毎朝6:45頃の実行として登録
