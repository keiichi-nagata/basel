...
- [x] エリア区分を楽天トラベルの提携エリア（12区分）に合わせて確定、初回は**北海道**に決定
      （9月下旬からの紅葉需要・候補宿の厚みから。沖縄は候補が薄く台風シーズンのため真冬に後回し）
- [x] 楽天アフィリエイト・JTB旅行予約（A8）とも汎用リンクを取得・`affiliates.json`に登録
- [x] 北海道エリアのリサーチ・執筆（TOP5: ふる川／洸の謌／第一滝本館／湖の栖／ミリオーネ）
      → `dev/4-onsen/drafts/2026-10.md`（データは `data/2026-10.json`）
- [x] 価格は掲載しない方針に変更（変動が激しく確認の手間が大きいため）
- [x] TOP5の5宿分、楽天トラベル個別アフィリエイトリンクを発行して下書きに反映
- [x] マガジン名を「エリア別 温泉宿ランキング（月刊）」に決定
- [x] マガジン表紙・サムネイル画像を作成（明るいクリーム＋テラコッタ配色で他部と差別化）
- [ ] 社長レビュー（`sop/ranking-magazine-workflow.md` 手順3）→ noteでマガジン作成 → 公開
