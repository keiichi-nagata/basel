...
- ブロッカー: 楽天アフィリエイト未登録のため個別宿リンクは未対応。登録され次第リンク化
- 次: `dev/4-onsen/areas.md` で初回エリア（箱根想定）を確定 → 初月リサーチ
